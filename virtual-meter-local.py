...
def parse_message(client, userdata, message):
    ...
    print("signal from source= %s", signal)
...
def timer_fire():
    ...
    global outage_state
    print ("INTERRUPT TIMER:", round(time.time()) - round(last_call), "NUMBER OF OUTAGES: ", outage_counter )
    timer_check = time.time()
    if timer_check - last_call >= 3: #allowed time since last packet was recieved before entering critical condition.
        ...
        for times in range(5):
            ...
            time.sleep(1)
        timer_check = time.time()
    elif timer_check - last_call <= 3: outage_state = False
...
def mqttSignal():
    ...
    threading.Timer(5, timer_fire).start() #dontstart kill switch until booted for 5 seconds
    client.loop_forever()
